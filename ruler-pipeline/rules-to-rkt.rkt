...
;(define herbie-dir ".\\herbie\\")
(define herbie-dir "")
(define rule-file-input "rules.txt")
;(define rule-file-output "src\\syntax\\rules.rkt")
(define rule-file-output "../src/syntax/rules.rkt")


(struct rewrite (name lhs rhs))

(define split-all (λ(list)
                    (map split-rule list (build-list (length list) identity))))

(define split-rule (λ(rule index)
                     (local [(define split (string-split rule "=>"))]
                       ;(print rule) (print index) (print split)
                       (rewrite (number->string index) (string-trim (car split)) (string-trim (list-ref split 1))))))

;; we need to generate rules. Often won't have very good names I'm guessing... maybe we can number them and also generate a mappng of names to numbers.

#;
(define format-rule (λ(rw)
                      (rewrite (rewrite-name rw) (read (open-input-string (rewrite-lhs rw)))
                               (read (open-input-string (rewrite-rhs rw))))))

;(format-rule (split-rule "?a => (- ?a 0)" "1"))

;(print (read (open-input-string "(+ (- 5 2))")))
;; hmm, I should just output it as a string since we have syntax rules rather than functions

;;rx#"\?[\w]" we want to grab the words and delete the ? 
(define find-and-format-vars (λ(rules)
                               (local [(define find (λ(rule)
                                                      ;; return a list of matched items
                                                      (append (regexp-match* #px"\\?(\\w)" (rewrite-lhs rule) #:match-select second)
                                                              (regexp-match* #px"\\?(\\w)" (rewrite-rhs rule) #:match-select second))))]
                                 (remove-duplicates (foldr (λ(rule acc) (append acc (find rule))) empty rules)))))

;;(display (find-and-format-vars (split-all '("?a => (- ?a 0)" "(+ ?a ?b) => (+ ?b ?a)"))))

(define remove-questions (λ (rules)
                           (map (λ (rule) (rewrite (rewrite-name rule)
                                                   (string-replace (rewrite-lhs rule) "?" "")
                                                   (string-replace (rewrite-rhs rule) "?" ""))) rules)))
...
(define rules-as-strings (λ(rules)
                           (map
                            (λ(rule)
                              (string-append "[" (rewrite-name rule) " " (rewrite-lhs rule) " " (rewrite-rhs rule) "]")) rules)))

;(print (remove-questions '("?a => (- ?a 0)" "(+ ?a ?b) => (+ ?b ?a)")))
...
;; break strings by newline
;; put header around it with the find-and-format-vars out of split phase
(define write-header (λ (rules)
                       (string-append
                        "(define-ruleset* ruler-rules (ruler simplify)" "\n"
                        "#:type ("
                        (string-join (map (λ(var) (string-append "[" var " real]"))
                                          (find-and-format-vars rules)) " ")
                        ")")))
#;
(define create-ruleset (λ(rules-as-string-text)
                         (local [(define rules (string-split rules-as-string-text #rx"\n+"))]
                           (string-append (write-header (split-all rules))
                                          "\n"
                                          (process-rules rules)
                                          ")")
                           )))

(define create-ruleset (λ(rules-as-lines)
                         (local [(define rules rules-as-lines)]
                           (string-append (write-header (split-all rules))
                                          "\n"
                                          (process-rules rules)
                                          ")")
...
;(display (write-header (split-all '("?a => (- ?a 0)" "(+ ?a ?b) => (+ ?b ?a)"))))

#;
(display (create-ruleset
"?a => (- ?a 0)
           (+ ?a ?b) => (+ ?b ?a)"))


(define rules-rkt-prelude "#lang racket
...
(define remove-simplify (λ(herbie-rules)
                          (string-replace
                           (string-replace herbie-rules " simplify)" ")")
                           "simplify " " ")))


(define write-ruleset (λ (in-path out-path)
                        (call-with-output-file* out-path
                            (λ (out)
                              (display
                               (string-append
                                (if (equal? (vector "ruler-only")(current-command-line-arguments))
                                (remove-simplify rules-rkt-prelude)
                                rules-rkt-prelude)
                                (create-ruleset (file->lines in-path)))
                               out))
                            #:exists 'truncate)))

(print (current-command-line-arguments))
(write-ruleset rule-file-input rule-file-output)
